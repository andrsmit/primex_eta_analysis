...
void EtaAna::EtaggAnalysis() {
	...
	for(int ishow=0; ishow<m_nfcal; ishow++) {
		...
		/*
		double locTOFdx, locTOFdy, locTOFdt;
		CheckTOFMatch(locPos, locTOFdx, locTOFdy, locTOFdt, m_TOFRFCut);
		double locTOFdr = sqrt(pow(locTOFdx,2.0)+pow(locTOFdy,2.0));
		
		if((m_fcalE[ishow] > m_FCALEnergyCut) && (locTOFdr>8.0)) h_fcalRFdt->Fill(locT);
		*/
			...
			for(int igam=0; igam<(int)locGoodBeamPhotons.size(); igam++) {
				...
				if(isEta && isElastic) {
					if(locVetoOptions[1]) {
						...
						h_fcalRFdt->Fill(t1, fillWeight);
						h_fcalRFdt->Fill(t2, fillWeight);
					}
...
void EtaAna::InitHistograms() {
	...
	for(int iveto=0; iveto<m_nVetos; iveto++) {
		...
		h_pt[iveto] = new TH2F(Form("pt_veto_%d",iveto), 
			Form("Transverse Momentum Difference (Veto Option %d)",iveto), 650, 0., 6.5, 1000, -0.5, 0.5);
		...
		h_ptCoh[iveto] = new TH2F(Form("ptConstr_veto_%d",iveto), 
			Form("Transverse Momentum Differnce (Veto Option %d)",iveto), 650, 0., 6.5, 1000, -0.5, 0.5);
		...
		h_mm[iveto] = new TH2F(Form("mm_veto_%d",iveto), 
			Form("Missing Mass (Veto Option %d)",iveto), 650, 0., 6.5, 1000, 0.0, 20.0);
