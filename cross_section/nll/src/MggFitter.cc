...
void MggFitter::FitData()
{
	...
	excludeRegions.push_back({0.50,0.58});
	...
	*/
	...
	printf("  fit 5: all parameters floating...\n");
	...
	locFitter5.Config().MinimizerOptions().SetMaxFunctionCalls(100000);
	locFitter5.Config().MinimizerOptions().SetDefaultErrorDef(0.5);
	
	//locFitter5.Config().SetMinimizer("Minuit", "Simplex");
	//locFitter5.FitFCN();
	locFitter5.Config().SetMinimizer("Minuit", "Migrad");
	locFitter5.FitFCN();
	
	FixEmptyParameters(locFitter5);
	FixOmegaParameters(locFitter5);
	FixBkgdParameters(locFitter5);
	combinedFit = 0;
	...
	//locFitter5.Config().SetMinosErrors({yieldPar, etaPiPar});
	
	/*
	if(!ok) {
		std::cerr << "Migrad failed, retrying with all empty target parameters fixed...\n";
		FixEmptyParameters(locFitter5);
		ok = locFitter5.FitFCN();
	}
	if(!ok) {
	...
	*/
	...
	if(ok) {
		...
		locFitter5.CalculateHessErrors();
		
		locFitter5.CalculateMinosErrors();
