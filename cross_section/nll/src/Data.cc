...
int EtaAnalyzer::LoadDataHistograms()
{
	...
	TString fullTargetFileName  = Form("%s/phase%d/%s/CORRECTED_0.3PERCENT/%s/full_target_%s%s%s.root",  
		dataDirectory.Data(), m_phase, fileTypeString.Data(), 
		dirString.Data(), fieldString.Data(), anaString.Data(), vetoStr.Data());
	
	TString emptyTargetFileName = Form("%s/phase%d/%s/CORRECTED_0.3PERCENT/%s/empty_target_%s%s%s.root", 
		dataDirectory.Data(), m_phase, fileTypeString.Data(), 
		dirString.Data(), fieldString.Data(), anaString.Data(), vetoStr.Data());
	...
	h_mggVsThetaEmpty_acc->Scale(m_emptyTargetFluxRatio);
...
int EtaAnalyzer::LoadEtaLineshape()
{
	...
	TString mcFileName = Form("%s/phase%d/UNCORRECTED/phase%d%s%s.root",  etamcDirectory.Data(), locPhase, locPhase, anaString.Data(), vetoStr.Data());
