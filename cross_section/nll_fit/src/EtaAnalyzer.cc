...
{
	return "Combined fit with full-target data";
}
...
void EtaAnalyzer::DrawFitResult(TH1F *h1, TH1F *hPull, TF1 *fFit, TF1 *fSignal, 
	TF1 *fEmpty, TF1 *fEtaPi, TF1 *fHadronicBkgd, TF1 *fOmega, TF1 *fRho, TF1 *fBkgd, TF1 *fAcc,
...
void EtaAnalyzer::ExtractAngularYield(MggFitter &locFitter, int drawOption)
{
	...
	for(int iThetaBin=0; iThetaBin<m_angularBin.size(); iThetaBin++)
	{
		...
		*/
		...
		// This code assumes the lineshape is constant over a wide range of theta.
		...
		locFitter.SetEmpty(locHistEmpty, locHistEmpty_acc, m_emptyTargetFluxRatio, 0.02*m_emptyTargetFluxRatio);
		locFitter.SetEmptyWide(locHistEmptyWide, locHistEmptyWide_acc, locEmptyRatio);
		...
		// Get fitted yields for each background source integrated within mgg cut:
		
		double locBkgdYield, locBkgdYieldErr;
		
		locFitter.GetEmptyYield(locBkgdYield, locBkgdYieldErr);
		m_angularYield_Empty[iThetaBin] = {locBkgdYield, locBkgdYieldErr};
		
		locFitter.GetEtaPionYield(locBkgdYield, locBkgdYieldErr);
		m_angularYield_EtaPion[iThetaBin] = {locBkgdYield, locBkgdYieldErr};
		
		locFitter.GetHadronicBkgdYield(locBkgdYield, locBkgdYieldErr);
		m_angularYield_HadronicBkgd[iThetaBin] = {locBkgdYield, locBkgdYieldErr};
		
		locFitter.GetOmegaYield(locBkgdYield, locBkgdYieldErr);
		m_angularYield_Omega[iThetaBin] = {locBkgdYield, locBkgdYieldErr};
		
		locFitter.GetRhoYield(locBkgdYield, locBkgdYieldErr);
		m_angularYield_Rho[iThetaBin] = {locBkgdYield, locBkgdYieldErr};
		...
		locFitter.GetEtaPionBkgdFraction(locBkgdFrac, locBkgdFracErr);
		m_angularEtaPionBkgdFraction[iThetaBin] = {locBkgdFrac, locBkgdFracErr};
		
		locFitter.GetHadronicBkgdFraction(locBkgdFrac, locBkgdFracErr);
		m_angularHadronicBkgdFraction[iThetaBin] = {locBkgdFrac, locBkgdFracErr};
		...
		if(drawOption)
		{
			bool locDrawOption_Empty = true;
			bool locDrawOption_Acc   = false;
			...
			TF1 *locfSignal       = nullptr;
			TF1 *locfEmpty        = nullptr;
			TF1 *locfEtaPion      = nullptr;
			TF1 *locfHadronicBkgd = nullptr;
			...
			locFitter.GetFitFunction_EtaPion(&locfEtaPion, "locfEtaPion");
			locFitter.GetFitFunction_HadronicBkgd(&locfHadronicBkgd, "locfHadronicBkgd");
			locFitter.GetFitFunction_Omega(&locfOmega, "locfOmega");
			...
			DrawFitResult(locHistFull, locPull, locfFit, locfSignal, 
				locfEmpty, locfEtaPion, locfHadronicBkgd, locfOmega, locfRho, locfBkgd, locfAcc,
				locMinAngle, locMaxAngle);
			
			if(locDrawOption_Empty)
			{
			...
			{
				// some code to visually check the fitting of the expected counts in the accidental background histogram:
				
				TH1F *hAccRatioFull, *hAccRatioEmpty;
				locFitter.GetAccFitRatio(&hAccRatioFull,  0, "accRatioFull");
				locFitter.GetAccFitRatio(&hAccRatioEmpty, 1, "accRatioEmpty");
				
				TCanvas *cAccRatioFull = new TCanvas("cAccRatioFull", "Acc Ratio (full)", 1000, 700);
				cAccRatioFull->SetTickx(); cAccRatioFull->SetTicky();
				cAccRatioFull->SetLeftMargin(0.13); cAccRatioFull->SetRightMargin(0.07);
				cAccRatioFull->SetBottomMargin(0.13); cAccRatioFull->SetTopMargin(0.07);
				cAccRatioFull->SetGrid();
				cAccRatioFull->cd();
				hAccRatioFull->Draw("hist");
				cAccRatioFull->Update();
				cAccRatioFull->Modified();
				
				TCanvas *cAccRatioEmpty = new TCanvas("cAccRatioEmpty", "Acc Ratio (empty)", 1000, 700);
				cAccRatioEmpty->SetTickx(); cAccRatioEmpty->SetTicky();
				cAccRatioEmpty->SetLeftMargin(0.13); cAccRatioEmpty->SetRightMargin(0.07);
				cAccRatioEmpty->SetBottomMargin(0.13); cAccRatioEmpty->SetTopMargin(0.07);
				cAccRatioEmpty->SetGrid();
				cAccRatioEmpty->cd();
				hAccRatioEmpty->Draw("hist");
				cAccRatioEmpty->Update();
				cAccRatioEmpty->Modified();
				
				getchar();
				
				delete hAccRatioFull;
				delete cAccRatioFull;
				delete hAccRatioEmpty;
				delete cAccRatioEmpty;
			}
		...
		delete locHistFull;
		delete locHistFull_acc;
		delete locHistEmpty;
		delete locHistEmpty_acc;
		delete locHistEmptyWide;
		delete locHistEmptyWide_acc;
		delete hEtaCoh;
		delete hEtaQF;
		delete hEtaPionBkgd;
...
void EtaAnalyzer::PlotEmptyEtaRatio()
{
	...
	for(int ibin=0; ibin<m_angularBin.size(); ibin++) {
		...
		double den    = (m_angularYieldFit[ibin].first + m_angularYield_HadronicBkgd[ibin].first + m_angularYield_EtaPion[ibin].first);
		double denErr = m_angularYield[ibin].second * (den/m_angularYield[ibin].first);
		
		double locRatio = 0., locRatioErr = 0.;
		if(den>0) {
			locRatio    = num/den;
			locRatioErr = sqrt(pow(numErr/den,2.0) + pow(denErr*num/den/den,2.0));
		}
...
void EtaAnalyzer::PlotBackgrounds()
{
	...
	StyleYieldHistogram(h_RhoYield,          4, kYellow);
	...
	h_RhoYield->GetYaxis()->SetTitle(Form("counts / %.2f#circ", 2.0*m_angularBin[0].second));
	...
	for(int ibin=0; ibin<m_angularBin.size(); ibin++) 
	{
		...
		h_EmptyYield->SetBinContent(ibin+1, m_angularYield_Empty[ibin].first);
		h_EmptyYield->SetBinError(ibin+1, m_angularYield_Empty[ibin].second);
		
		h_EtaPionYield->SetBinContent(ibin+1, m_angularYield_EtaPion[ibin].first);
		h_EtaPionYield->SetBinError(ibin+1, m_angularYield_EtaPion[ibin].second);
		
		h_HadronicBkgdYield->SetBinContent(ibin+1, m_angularYield_HadronicBkgd[ibin].first);
		h_HadronicBkgdYield->SetBinError(ibin+1, m_angularYield_HadronicBkgd[ibin].second);
		
		h_OmegaYield->SetBinContent(ibin+1, m_angularYield_Omega[ibin].first);
		h_OmegaYield->SetBinError(ibin+1, m_angularYield_Omega[ibin].second);
		
		h_RhoYield->SetBinContent(ibin+1, m_angularYield_Rho[ibin].first);
		h_RhoYield->SetBinError(ibin+1, m_angularYield_Rho[ibin].second);
	...
	legBg->AddEntry(h_Yield,             "Single-#eta Yield",                     "PE1X0");
	legBg->AddEntry(h_EtaPionYield,      "#eta#pi Bkgd",                          "PE1X0");
	legBg->AddEntry(h_HadronicBkgdYield, "#eta#pi#pi Bkgd",                       "PE1X0");
	legBg->AddEntry(h_OmegaYield,        "#omega#rightarrow#pi^{0}#gamma Bkgd",   "PE1X0");
	legBg->AddEntry(h_RhoYield,          "#rho^{0}#rightarrow#pi^{0}#gamma Bkgd", "PE1X0");
...
void EtaAnalyzer::InitializeBinning()
{
	...
	while(locAngle < m_maxReconAngle) 
	{
		...
		m_angularYield_Empty.push_back({0.0, 0.0});
		m_angularYield_EtaPion.push_back({0.0, 0.0});
		m_angularYield_HadronicBkgd.push_back({0.0, 0.0});
		m_angularYield_Omega.push_back({0.0, 0.0});
		m_angularYield_Rho.push_back({0.0, 0.0});
		m_angularYield_Bkgd.push_back({0.0, 0.0});
...
void InitializeMggFitter(MggFitter &fitter, EtaAnalyzer *anaObj, double angle, double angleWidth)
{
	...
	fitter.fitOption_rho    = anaObj->GetFitOption(5);
	fitter.fitOption_etap   = anaObj->GetFitOption(6);
...
void EtaAnalyzer::WriteROOTFile(TString fileName)
{
	...
	if(h_RhoYield) h_RhoYield->Write();
