...
int main(int argc, char **argv) 
{
	...
	locYieldFitter.SetModel(SGEVORKYAN_UPD_V2);
	...
	if(0)
	{
		...
		TH1F *hAcc = (TH1F*)fIn->Get("h_Acceptance");
		
		if(1) {
			hYield = (TH1F*)fIn->Get("AngularYieldFit");
			hCS    = (TH1F*)fIn->Get("CrossSectionFit");
		}
		else {
			hYield = (TH1F*)fIn->Get("AngularYieldInclusive");
			hCS    = (TH1F*)fIn->Get("CrossSectionInclusive");
			/*
			hYield = (TH1F*)fIn->Get("AngularYieldFit");
			TH1F *hb1 = (TH1F*)fIn->Get("EtaPionYield")->Clone("hb1");
			hYield->Add(hb1);
			if(locEtaAna.GetVetoOption()<6) {
				TH1F *hb2 = (TH1F*)fIn->Get("HadronicBkgdYield")->Clone("hb2");
				hYield->Add(hb2);
			}
			
			TH1F *hEtaPi = (TH1F*)fIn->Get("EtaPionYield")->Clone("hEtaPi");
			TF1  *fEtaPi = new TF1("fEtaPi", "pol5", 0.0, 4.5);
			fEtaPi->SetParLimits(0, 0.0, 1.e3);
			fEtaPi->FixParameter(1, 0.0);
			hEtaPi->Fit(fEtaPi, "R0QL");
			
			TH1F *hEtaPiPi = (TH1F*)fIn->Get("HadronicBkgdYield")->Clone("hEtaPiPi");
			TF1  *fEtaPiPi = new TF1("fEtaPiPi", "pol5", 0.0, 4.5);
			fEtaPiPi->SetParLimits(0, 0.0, 1.e3);
			fEtaPiPi->FixParameter(1, 0.0);
			if(locEtaAna.GetVetoOption()<6) hEtaPiPi->Fit(fEtaPiPi, "R0QL");
			
			TCanvas *cBkgd = new TCanvas("cBkgd","cBkgd",1000,400);
			cBkgd->Divide(2,1);
			cBkgd->cd(1);
			hEtaPi->Draw("PE1X0");
			fEtaPi->Draw("same");
			...
			for(int ibin=1; ibin<=hYield->GetXaxis()->GetNbins(); ibin++) {
				double locYield = hYield->GetBinContent(ibin);
				double locBG1   = fEtaPi->Eval(hYield->GetBinCenter(ibin));
				double locBG2   = locEtaAna.GetVetoOption()<6 ? fEtaPiPi->Eval(hYield->GetBinCenter(ibin)) : 0.0;
				//double locBG1   = locEtaAna.GetVetoOption()<6 ?   hEtaPi->GetBinContent(ibin) : 0.0;
				//double locBG2   = locEtaAna.GetVetoOption()<6 ? hEtaPiPi->GetBinContent(ibin) : 0.0;
				hYield->SetBinContent(ibin, locYield - locBG1 - locBG2);
			}
			*/
		}
		...
		locYieldFitter.SetCS(hCS);
		locYieldFitter.SetAcc(hAcc);
	}
	...
	double minFitRange = 0.0, maxFitRange = 2.5;
