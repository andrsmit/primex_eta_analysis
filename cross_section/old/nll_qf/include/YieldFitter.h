...
class YieldFitter {
	public:
		...
		double YieldDrawFunction_dOmega(double *x, double *par);
		double YieldDrawFunctionInterference_dOmega(double *x, double *par);
		double CSDrawFunction(double *x, double *par);
		double CSDrawFunctionInterference(double *x, double *par);
		...
		void SetCS(TH1F *h1) { h_cs = (TH1F*)h1->Clone("locCrossSection"); return; }
		void SetAcc(TH1F *h1) { h_acc = h1; return; }
		
		int LoadTheoryHists();
		void FitAngularYield(double minFitRange=0.0, double maxFitRange=4.0);
		void CombinedFit(double minFitRange=0.0, double maxFitRange=4.0);
	...
	private:
		...
		// Angular Yield Distributions subdivided into smaller energy bins:
		TH1F *h_yield_8GeV, *h_yield_9GeV, *h_yield_10GeV;
		TF1  *f_yield_8GeV, *f_yield_9GeV, *f_yield_10GeV;
		...
		void InitializeDrawFunction_dOmega(TF1 **f1, TString funcName, int lineColor=kBlack, int lineStyle=1, int lineWidth=2);
		void InitializeInterFunction(TF1 **f1, TString funcName, int lineColor=kBlack, int lineStyle=1, int lineWidth=2);
		void InitializeInterFunction_dOmega(TF1 **f1, TString funcName, int lineColor=kBlack, int lineStyle=1, int lineWidth=2);
		void DrawFitResult(double min=0.0, double max=4.0);
		
		void InitializeCSFunction(TF1 **f1, TString funcName, int lineColor=kBlack, int lineStyle=1, int lineWidth=2);
		void InitializeCSFunctionInterference(TF1 **f1, TString funcName, int lineColor=kBlack, int lineStyle=1, int lineWidth=2);
};
