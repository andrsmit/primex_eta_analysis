...
int main(int argc, char **argv) 
{
	...
	double minFitRange = 0.0, maxFitRange = 2.2;
...
int LoadConfigSettings(EtaAnalyzer &anaObject, TString fileName, int phase)
{
	...
	*/
	...
	switch(anaObject.GetAnalysisOption()) {
		case 0:
			anaObject.SetMggHistName(Form("VetoOption%d/hmassCorr_veto_%d",
	...
	if(ReadFile->GetConfigName("rebinsHmass") != "") {
		anaObject.SetRebinsHmass(ReadFile->GetConfig1Par("rebinsHmass")[0]);
	}
	...
	if(!anaObject.GetEmptySubtractOption())
	{
		...
		if(ReadFile->GetConfigName("rebinsEmptyHmass") != "") {
			anaObject.SetRebinsEmptyHmass(ReadFile->GetConfig1Par("rebinsEmptyHmass")[0]);
		}
	}
