...
void MggFitter::FitData()
{
	...
	if(fitOption_omega>0) {
		double minOmegaFit = 0.15;
		double maxOmegaFit = 0.40;
	...
	//double deltaMuGuess = 0.00222143 + (-0.00211299*angle) + (0.00148088*pow(angle,2.0)) + (-0.000213241*pow(angle,3.0));
	
	// shift fitted from veto option 7 with hadronic bkgd fractions floating and using bggen lineshape above 1.5 degrees:
	double deltaMuGuess = 0.00131291 + (-5.99712e-05*angle) + (0.000128043*pow(angle,2.0));
	
	//f_fit->FixParameter(deltaMuPar,deltaMuGuess);
	
	f_fit->FixParameter(deltaMuPar, 0.004);
	...
	if(fitOption_empty) {
		//if(emptyFitOption_eta>0)   f_empty->SetParameter("N_{#eta}",   0.0);
		//if(emptyFitOption_omega>0) f_empty->SetParameter("N_{#omega}", 0.0);
		//f_empty->SetParameter("N_{fdc,2}", 0.0);
	...
	if(fitOption_signal>10) {
		...
		f_fit->SetRange(-0.1, maxFitRange);
		
		etaPionBkgdFracPar = f_fit->GetParNumber("A_{#eta#pi}");
		hadronicBkgdFracPar = f_fit->GetParNumber("A_{bkgd}");
		
		printf("\n\nETAPIONYIELD: %f\n", m_etaPionYieldBGGEN);
		printf("HADRONICBKGDYIELD: %f\n", m_hadronicBkgdYieldBGGEN);
		printf("ETAYIELD: %f\n\n", f_fit->GetParameter("N_{#eta}"));
		
		//if(m_etaPionYieldBGGEN<(0.02*f_fit->GetParameter("N_{#eta}"))) {
			//f_fit->FixParameter(etaPionBkgdFracPar, 1.0);
		//} else {
			f_fit->ReleaseParameter(etaPionBkgdFracPar);
			f_fit->SetParLimits(etaPionBkgdFracPar, 0.0, 10.0);
		//}
		if(m_hadronicBkgdYieldBGGEN<(0.05*f_fit->GetParameter("N_{#eta}"))) {
...
void MggFitter::GuessOmegaParameters()
{
	...
	double    muGuess = 0.285;
	double sigmaGuess = 0.020;
	double alphaGuess = 1.0;
	double     nGuess = 6.0;
	
	double locMax = 0.0;
	for(int ibin=h_data->FindBin(0.20); ibin<=h_data->FindBin(maxOmegaFit); ibin++) {
	...
	switch(fitOption_omega) {
		case 1:
		{
			...
			f_fit->SetParLimits(   muPar, 0.230,  0.300);
			f_fit->SetParLimits(sigmaPar, 0.005,  0.050);
		...
		case 5:
		{
			...
			f_fit->SetParLimits(muPar,  0.20, 0.40);
...
void MggFitter::GuessEtaParameters()
{
	...
	switch(fitOption_signal) {
		case 1:
		{
			...
			f_fit->SetParLimits(    NPar,  0.000, 1.0e5);
			f_fit->SetParLimits(   muPar, -0.020, 0.020);
			f_fit->SetParLimits(sigmaPar,  0.005, 0.030);
			break;
		}
		case 2:
		{
...
void MggFitter::FitEtaLineshape(int drawOption)
{
	...
	TF1 *locfEta = new TF1("locfEta", DoubleCrystalBallPlusGausPDF, -0.10, 0.10, 13);
	locfEta->SetParameter(0, 0.00);  // mu1
	...
	locfEta->SetParLimits(0, -0.01,  0.02);
	...
	if(drawOption) {
		...
		//gStyle->SetOptFit(0);
		h_etaLineshape->GetXaxis()->SetRangeUser(-0.2,0.2);
		h_etaLineshape->Draw();
		locfEta->SetRange(-0.2,0.2);
		locfEta->SetNpx(1000);
		locfEta->Draw("same");
		//h_etaLineshape->Draw("same");
		
		double fracAccepted = locfEta->Integral(-0.1,0.1) / h_etaLineshape->GetXaxis()->GetBinWidth(1);
		printf("  fraction of signal lineshape within hmass cut: %f\n", fracAccepted);
...
{
	if(h_hadronicBkgdLineshape==NULL) return;
	
	TF1 *fHadronicBkgd1 = new TF1("fHadronicBkgd1", CrystalBallPDF_flip, -0.1, 0.25, 5);
	...
	fHadronicBkgd1->SetParLimits(0, 0.000,  0.050);
	...
	TF1 *fHadronicBkgd2 = new TF1("fHadronicBkgd2", DoubleCrystalBallPDF_flip, -0.1, 0.25, 10);
	...
	fHadronicBkgd2->SetParLimits(0,  0.000,  0.050);
	...
	if(drawOption) {
		...
		double fracAccepted = fHadronicBkgd2->Integral(-0.1,0.1) / h_hadronicBkgdLineshape->GetXaxis()->GetBinWidth(1);
		printf("  fraction of hadronic bkgd lineshape within hmass cut: %f\n", fracAccepted);
...
void MggFitter::FitEtaPionLineshape(int drawOption)
{
	...
	TF1 *fEtaPion1 = new TF1("fEtaPion1", CrystalBallPDF_flip, -0.1, 0.25, 5);
	...
	fEtaPion1->SetParLimits(0, 0.000,  0.050);
	...
	TF1 *fEtaPion2 = new TF1("fEtaPion2", DoubleCrystalBallPDF_flip, -0.1, 0.25, 10);
	...
	fEtaPion2->SetParLimits(0,  0.000,  0.050);
	...
	if(drawOption) {
		...
		double fracAccepted = fEtaPion2->Integral(-0.1,0.1) / h_etaPionLineshape->GetXaxis()->GetBinWidth(1);
		printf("  fraction of eta+pion lineshape within hmass cut: %f\n", fracAccepted);
...
void MggFitter::FitOmegaLineshape(int drawOption)
{
	...
	TF1 *fOmega1 = new TF1("fOmega1", CrystalBallPDF, 0.0, 0.45, 5);
	...
	double locMax = 0.0;
	for(int ibin=h_omegaLineshape->FindBin(0.25); ibin<=h_omegaLineshape->FindBin(0.45); ibin++) {
	...
	fOmega1->SetParLimits(0, 0.250,  0.450);
	...
	TF1 *fOmega2 = new TF1("fOmega2", DoubleCrystalBallPDF, 0.0, 0.45, 10);
	...
	fOmega2->SetParLimits(0,  0.250,  0.450);
		...
		switch(fitOption_signal) {
			...
			case 7:
			{
				...
				*/
				...
				double locCorrection_bkgd = f_hadronicBkgdLineshape->Integral(minMggCut, maxMggCut);
			...
			case 8:
			{
				...
				*/
				...
				double locCorrection_bkgd = h_hadronicBkgdLineshape->Integral(h_hadronicBkgdLineshape->FindBin(minMggCut), 
					h_hadronicBkgdLineshape->FindBin(maxMggCut));
				double locYieldHadronicBkgd    = locCorrection_bkgd * N_bkgd;
				double locYieldHadronicBkgdErr = locCorrection_bkgd * N_bkgdErr;
			...
			case 9:
			{
				...
				*/
				...
				double locCorrection  = h_etaLineshape->Integral(h_etaLineshape->FindBin(minMggCut), h_etaLineshape->FindBin(maxMggCut));
				double locYieldEta    = locCorrection * N_eta;
				double locYieldEtaErr = locCorrection * N_etaErr;
				
				double locCorrectionEtaPion = h_etaPionLineshape->Integral(h_etaPionLineshape->FindBin(minMggCut), 
					h_etaPionLineshape->FindBin(maxMggCut));
				double locYieldEtaPion    = locCorrectionEtaPion * N_etapi;
				double locYieldEtaPionErr = locCorrectionEtaPion * N_etapiErr;
				
				double locCorrectionBkgd = h_hadronicBkgdLineshape->Integral(h_hadronicBkgdLineshape->FindBin(minMggCut), 
					h_hadronicBkgdLineshape->FindBin(maxMggCut));
			...
			case 10:
			{
				...
				*/
				...
				double locCorrectionEtaPion = f_etaPionLineshape->Integral(minMggCut, maxMggCut);
				double locYieldEtaPion      = locCorrectionEtaPion * N_etapi;
				double locYieldEtaPionErr   = locCorrectionEtaPion * N_etapiErr;
			...
			case 11:
			{
				...
				int yieldParBkgd  = f_fit->GetParNumber("A_{bkgd}");
...
void MggFitter::GetHadronicBkgdYield(double &yield, double &yieldErr)
{
	...
	for(int ipar=0; ipar<nParameters; ipar++) {
		...
		if(locParName=="A_{bkgd}")         continue;
		if(locParName.Contains("sigma"))   continue;
	...
	int bkgdYieldPar;
	if(fitOption_signal>10) bkgdYieldPar = f_fit->GetParNumber("A_{bkgd}");
...
void MggFitter::GetHadronicBkgdFraction(double &fraction, double &fractionErr)
{
	...
	int fractionPar;
	if(fitOption_signal==11) fractionPar = f_fit->GetParNumber("A_{bkgd}");
...
void MggFitter::GetEtaPionBkgdFraction(double &fraction, double &fractionErr)
{
	...
	int fractionPar;
	if(fitOption_signal==11) fractionPar = f_fit->GetParNumber("A_{#eta#pi}");
