...
void MggFitter::FitEmpty()
{
	...
	excludeRegions.push_back({-0.10,0.10});
	excludeRegions.push_back({ 0.20,0.45});
...
void MggFitter::GuessEmptyEtaParameters() {
	...
	switch(emptyFitOption_eta) {
		...
		case 1:
		{
			...
			f_empty->SetParLimits(   muPar, -0.01, 0.01);
...
void MggFitter::GuessEmptyOmegaParameters() {
	...
	switch(emptyFitOption_omega) {
		...
		case 1:
		{
			...
			f_empty->SetParameter(   muPar, 0.280);
			...
			f_empty->SetParLimits(   muPar, 0.230,  0.300);
