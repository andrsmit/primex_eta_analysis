...
int EtaAnalyzer::LoadEtaLineshape()
{
	...
	if(m_analysisOption==1) {
		...
		if(m_analysisOption==0) {
			h_etaLineshapeCoh = (TH2F*)mcFile->Get(Form("VetoOption%d/hmassCorr_veto_%d", 
...
{
	TString anaString = "";
	TString locMggHistName = "hmass_bggen_signal";
...
{
	TString anaString = "";
	TString locMggHistName1 = "hmass_bggen_";
	TString locMggHistName2 = "";
...
int EtaAnalyzer::LoadOmegaLineshape()
{
	...
	int locVetoOption = 4;
	if((m_vetoOption==1) || (m_vetoOption==7)) locVetoOption = m_vetoOption;
	...
	h_omegaLineshape = (TH2F*)mcFile->Get("hmass_bggen_omega")->Clone("omegaLineshape");
	h_omegaLineshape->SetDirectory(0);
	
	h_rhoLineshape = (TH2F*)mcFile->Get("hmass_bggen_rho")->Clone("rhoLineshape");
	...
	if(m_fitOption_omega==3) {
		h_omegaLineshape->Add(h_rhoLineshape);
		TH2F *loc_h2_bkgd = (TH2F*)mcFile->Get("hmass_bggen_bkgd")->Clone("bkgdLineshape");
